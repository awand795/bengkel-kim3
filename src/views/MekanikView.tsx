    ...
    <div className="space-y-5 max-w-4xl mx-auto pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base sm:text-lg font-bold text-slate-900">Mekanik Bengkel (Lembar Kerja)</h1>
              <span className="px-2.5 py-0.5 rounded-full bg-teal-50 text-teal-800 text-[11px] font-bold border border-teal-200/70">
                PIT AREA
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-0.5">Mekanik: <strong className="text-slate-700">{currentUser}</strong> | Stopwatch Live Monitoring</p>
          </div>
        </div>

        {/* Live Job Timer Badge */}
        <div className="flex items-center justify-between flex-wrap gap-2 bg-slate-900 text-white px-4 py-2.5 rounded-xl font-mono text-xs sm:text-sm font-bold shadow-xs mt-3">
          ...
          <div className="shrink-0 max-w-full">
            {timerRunning ? (
              <span className="text-[10px] uppercase font-bold tracking-wider bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                ● Berjalan
              </span>
            ) : myJob?.status_spk === 'Waiting Part' || myJob?.status_spk === 'Pending' ? (
              <span className="text-[10px] uppercase font-bold tracking-wider bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2.5 py-0.5 rounded-full animate-pulse whitespace-nowrap">
                Auto-Paused ({myJob.status_spk})
              </span>
            ) : isManualPaused ? (
              <span className="text-[10px] uppercase font-bold tracking-wider bg-white/10 text-slate-300 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                Dijeda Manual
              </span>
            ) : (
              <span className="text-[10px] uppercase font-bold tracking-wider bg-white/10 text-slate-400 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                Siap
              </span>
            )}
          </div>
      ...
      {myJob ? (
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-xs space-y-5">
          ...
          {/* WO Header Banner */}
          <div className="p-5 rounded-2xl bg-linear-to-r from-teal-900 via-teal-800 to-slate-900 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm border border-teal-700/40">
            <div className="min-w-0">
              <span className="text-[10px] text-teal-200 uppercase tracking-widest font-bold">Active Work Order</span>
              <div className="text-xl sm:text-2xl font-bold font-mono mt-0.5 break-all text-white">{myJob.no_spk}</div>
              <div className="text-xs text-teal-100 font-medium truncate mt-0.5">{myJob.no_polisi} - {myJob.nama_customer}</div>
            </div>
            <div className="flex items-center gap-2 flex-wrap shrink-0">
              <button
                type="button"
                onClick={() => setShowPrintSpk(myJob)}
                className="px-3.5 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs flex items-center gap-1.5 transition-colors backdrop-blur-md border border-white/10"
