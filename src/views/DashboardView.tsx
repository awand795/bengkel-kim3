...
export const DashboardView: React.FC = () => {
  const { currentRole, currentUser, setActiveTab, setRole } = useAppStore();
  ...
  const renderSaDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Banner */}
        <div className="rounded-2xl bg-gradient-to-r from-blue-700 via-indigo-700 to-blue-800 text-white p-5 sm:p-6 shadow-md relative overflow-hidden">
          <div className="relative z-10">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-blue-900/80 hover:bg-blue-900 text-white font-semibold text-xs border border-blue-400/30 transition-all flex items-center gap-1.5"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <div 
            onClick={() => setActiveTab('sa')}
            className="bg-white rounded-2xl p-4 border border-blue-200 shadow-xs hover:shadow-md transition-all cursor-pointer group"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-blue-500 text-white flex items-center justify-center">
                <Truck className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded-md">Gate Check-in</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{antrianMenunggu.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Menunggu Penerimaan SA</div>
          </div>

          <div 
            onClick={() => setActiveTab('sa')}
            className="bg-white rounded-2xl p-4 border border-amber-200 shadow-xs hover:shadow-md transition-all cursor-pointer group"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-amber-500 text-white flex items-center justify-center">
                <ClipboardList className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 px-2 py-0.5 rounded-md">Proses Servis</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkAktif.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">SPK Aktif di Bengkel</div>
          </div>

          <div 
            onClick={() => setActiveTab('purchasing')}
            className="bg-white rounded-2xl p-4 border border-purple-200 shadow-xs hover:shadow-md transition-all cursor-pointer group"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-purple-500 text-white flex items-center justify-center">
                <ShoppingBag className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-purple-600 uppercase bg-purple-50 px-2 py-0.5 rounded-md">Kotak Merah</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{prKotakMerah.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Part PO / Approval SA</div>
          </div>

          <div 
            onClick={() => setActiveTab('sa')}
            className="bg-white rounded-2xl p-4 border border-emerald-200 shadow-xs hover:shadow-md transition-all cursor-pointer group"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-emerald-500 text-white flex items-center justify-center">
                <CheckCircle2 className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-emerald-600 uppercase bg-emerald-50 px-2 py-0.5 rounded-md">Siap Serah Terima</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkSiapQc.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">QC &amp; FIR Closed</div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Unit telah melalui gate check-in dan siap dibuatkan formulir SPK</p>
              </div>
              <button
                onClick={() => setActiveTab('sa')}
                  ...
                  {antrianMenunggu.length > 0 ? (
                    antrianMenunggu.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3 px-3 font-mono font-bold text-blue-600">{item.no_tiket}</td>
                        ...
                        <td className="py-3 px-3 text-right">
                          <button
                            onClick={() => setActiveTab('sa')}
                            className="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold text-[11px] transition-colors inline-flex items-center gap-1 shadow-xs"
                          >
              ...
              <div className="space-y-3 mt-4">
                {prKotakMerah.length > 0 ? (
                  prKotakMerah.slice(0, 3).map((item) => (
                    <div key={item.pr_id} className="p-3 rounded-xl border border-purple-200 bg-purple-50/40 space-y-1.5">
                      ...
                      {item.estimasi_tanggal_ready_eta && (
                        <div className="pt-1.5 border-t border-purple-200/60 flex items-center justify-between text-[10px] text-purple-900 font-semibold">
  ...
  const renderForemanDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Banner */}
        <div className="rounded-2xl bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 text-white p-5 sm:p-6 shadow-md relative overflow-hidden">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[11px] font-semibold mb-2 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-white text-amber-700 font-bold text-xs shadow-sm hover:bg-amber-50 transition-all flex items-center gap-1.5"
              >
                <Wrench className="w-3.5 h-3.5" /> Penugasan Mekanik &amp; QC
              </button>
              <button
                onClick={() => setActiveTab('mekanik')}
                className="px-3 py-1.5 rounded-xl bg-amber-800/80 hover:bg-amber-800 text-white font-semibold text-xs border border-amber-300/30 transition-all flex items-center gap-1.5"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <div 
            onClick={() => setActiveTab('foreman')}
            className="bg-white rounded-2xl p-4 border border-rose-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-rose-500 text-white flex items-center justify-center">
                <AlertCircle className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-rose-600 uppercase bg-rose-50 px-2 py-0.5 rounded-md">Butuh Assign</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkMenungguAssign.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">SPK Menunggu Mekanik</div>
          </div>

          <div 
            onClick={() => setActiveTab('foreman')}
            className="bg-white rounded-2xl p-4 border border-amber-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-amber-500 text-white flex items-center justify-center">
                <Wrench className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 px-2 py-0.5 rounded-md">Stall Aktif</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkSedangDikerjakan.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Sedang Dikerjakan</div>
          </div>

          <div 
            onClick={() => setActiveTab('foreman')}
            className="bg-white rounded-2xl p-4 border border-sky-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-sky-500 text-white flex items-center justify-center">
                <CheckCircle className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-sky-600 uppercase bg-sky-50 px-2 py-0.5 rounded-md">Inspeksi</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkMenungguQc.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Antrian QC &amp; FIR</div>
          </div>

          <div 
            onClick={() => setActiveTab('foreman')}
            className="bg-white rounded-2xl p-4 border border-purple-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-purple-500 text-white flex items-center justify-center">
                <AlertTriangle className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-purple-600 uppercase bg-purple-50 px-2 py-0.5 rounded-md">Job Tambahan</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{tambahanPending.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Pekerjaan Tambahan</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Daftar SPK aktif dan alokasi mekanik penanggung jawab</p>
              </div>
              <button
                onClick={() => setActiveTab('foreman')}
                    ...
                    spkList.slice(0, 5).map((spk) => (
                      <tr key={spk.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3 px-3 text-right">
                          <button
                            onClick={() => setActiveTab('foreman')}
                            className="px-2.5 py-1 rounded-lg bg-amber-500 hover:bg-amber-600 text-white font-bold text-[11px] transition-colors inline-flex items-center gap-1"
                          >
                            Kelola SPK
                          </button>
                        </td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between">
              ...
              <div className="space-y-3 mt-4">
                {spkMenungguQc.length > 0 ? (
                  spkMenungguQc.map((spk) => (
                    <div key={spk.id} className="p-3 rounded-xl border border-sky-200 bg-sky-50/40 space-y-1.5">
                      <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => setActiveTab('foreman')}
              className="w-full mt-4 py-2 px-3 rounded-xl bg-sky-600 text-white font-bold text-xs hover:bg-sky-700 transition-colors shadow-xs flex items-center justify-center gap-2"
            >
  ...
  const renderMekanikDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Banner */}
        <div className="rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 text-white p-5 sm:p-6 shadow-md relative overflow-hidden">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[11px] font-semibold mb-2 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              <button
                onClick={() => setActiveTab('mekanik')}
                className="px-3.5 py-2 rounded-xl bg-white text-emerald-700 font-bold text-xs shadow-sm hover:bg-emerald-50 transition-all flex items-center gap-1.5"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          <div 
            onClick={() => setActiveTab('mekanik')}
            className="bg-white rounded-2xl p-4 border border-emerald-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-emerald-500 text-white flex items-center justify-center">
                <Wrench className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-emerald-600 uppercase bg-emerald-50 px-2 py-0.5 rounded-md">Aktif</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{mySpks.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Work Order Saya</div>
          </div>

          <div 
            onClick={() => setActiveTab('mekanik')}
            className="bg-white rounded-2xl p-4 border border-blue-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-blue-500 text-white flex items-center justify-center">
                <CheckCircle2 className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded-md">Selesai</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkDone.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Selesai Dikerjakan</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 px-2 py-0.5 rounded-md">Live Timer</span>
            </div>
            <div className="text-2xl font-black text-slate-900">Ready</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Status Stopwatch</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-purple-600 uppercase bg-purple-50 px-2 py-0.5 rounded-md">Temuan</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{tambahanList.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Temuan Tambahan</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Klik pengerjaan untuk membuka stopwatch live timer</p>
              </div>
              <button
                onClick={() => setActiveTab('mekanik')}
            ...
            <div className="space-y-3">
              {mySpks.length > 0 ? (
                mySpks.map((spk) => (
                  <div key={spk.id} className="p-4 rounded-xl border border-slate-200 hover:border-emerald-300 transition-all bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs font-bold text-blue-600">{spk.no_spk}</span>
                      ...
                      <div className="text-[11px] text-slate-400">
                        Lead time estimasi: <span className="font-bold text-slate-700">{spk.lead_time_jam ? `${spk.lead_time_jam} Jam` : '4 Jam'}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => setActiveTab('mekanik')}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
            ...
            <ul className="text-xs text-slate-600 space-y-2.5">
              ...
              <li className="flex items-start gap-2">
                <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-[10px] font-bold shrink-0 mt-0.5">4</span>
  ...
  const renderPurchasingDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Banner */}
        <div className="rounded-2xl bg-gradient-to-r from-purple-700 via-indigo-700 to-purple-800 text-white p-5 sm:p-6 shadow-md relative overflow-hidden">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[11px] font-semibold mb-2 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              <button
                onClick={() => setActiveTab('purchasing')}
                className="px-3.5 py-2 rounded-xl bg-white text-purple-700 font-bold text-xs shadow-sm hover:bg-purple-50 transition-all flex items-center gap-1.5"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-rose-600 uppercase bg-rose-50 px-2 py-0.5 rounded-md">PR Baru</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{prDiajukan.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">PR Masuk dari SA</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-purple-600 uppercase bg-purple-50 px-2 py-0.5 rounded-md">Bandingkan Vendor</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{prDiproses.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Penawaran 2 Vendor</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded-md">Monitoring ETA</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{poDiterbitkan.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">PO Dalam Pengiriman</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-emerald-600 uppercase bg-emerald-50 px-2 py-0.5 rounded-md">Total PR</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{purchasingList.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Semua Pengadaan</div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Part yang tidak ready di stok gudang dan membutuhkan PO</p>
              </div>
              <button
                onClick={() => setActiveTab('purchasing')}
                ...
                <tbody className="divide-y divide-slate-100">
                  {purchasingList.length > 0 ? (
                    purchasingList.slice(0, 5).map((item) => (
                      <tr key={item.pr_id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3 px-3 font-mono font-bold text-purple-700">{item.no_pr}</td>
                        <td className="py-3 px-3">
                        ...
                        <td className="py-3 px-3 text-right">
                          <button
                            onClick={() => setActiveTab('purchasing')}
                            className="px-2.5 py-1 rounded-lg bg-purple-600 hover:bg-purple-700 text-white font-bold text-[11px] transition-colors"
                          >
                            Proses PO
                          </button>
                        </td>
            ...
            <div className="space-y-3">
              {poDiterbitkan.length > 0 ? (
                poDiterbitkan.map((po) => (
                  <div key={po.pr_id} className="p-3 rounded-xl border border-blue-200 bg-blue-50/40 space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-mono font-bold text-blue-700">{po.no_po || 'PO-2026'}</span>
                      <span className="text-[10px] font-bold text-blue-600 bg-blue-100 px-2 py-0.5 rounded">Vendor: {po.vendor_terpilih || 'Vendor Utama'}</span>
                    </div>
                    ...
                    <div className="text-[11px] text-slate-600 flex items-center justify-between pt-1 border-t border-blue-100">
                      <span>ETA Tiba:</span>
                      <span className="font-bold text-blue-900">{po.estimasi_tanggal_ready_eta || 'Hari ini'} {po.estimasi_jam_ready_eta}</span>
                    </div>
                  </div>
                ))
              ) : (
  ...
  const renderKasirDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Banner */}
        <div className="rounded-2xl bg-gradient-to-r from-emerald-700 via-teal-700 to-emerald-800 text-white p-5 sm:p-6 shadow-md relative overflow-hidden">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[11px] font-semibold mb-2 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              ...
              <button
                onClick={() => setActiveTab('beli-part')}
                className="px-3.5 py-2 rounded-xl bg-emerald-900/80 hover:bg-emerald-900 text-white font-semibold text-xs border border-emerald-400/30 transition-all flex items-center gap-1.5"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded-md">Siap Faktur</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{spkSiapFaktur.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">SPK FIR Closed</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-50 px-2 py-0.5 rounded-md">Belum Lunas</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{invoiceUnpaid.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Tagihan Unpaid</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-emerald-600 uppercase bg-emerald-50 px-2 py-0.5 rounded-md">Penerimaan</span>
            </div>
            <div className="text-2xl font-black text-slate-900">
              Rp {(totalKasHariIni / 1000000).toFixed(1)} jt
            </div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Kas Masuk Hari Ini</div>
          </div>

          <div 
            onClick={() => setActiveTab('kasir')}
            className="bg-white rounded-2xl p-4 border border-indigo-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-indigo-500 text-white flex items-center justify-center">
                <FileText className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-indigo-600 uppercase bg-indigo-50 px-2 py-0.5 rounded-md">Memo Keluar</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{summary?.total_selesai ?? 2}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Unit Selesai &amp; Keluar</div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Unit telah lulus uji QC jalan &amp; FIR closed oleh SA &amp; Foreman</p>
              </div>
              <button
                onClick={() => setActiveTab('kasir')}
                className="text-xs font-bold text-emerald-600 hover:text-emerald-700 flex items-center gap-1"
                  ...
                  {spkSiapFaktur.length > 0 ? (
                    spkSiapFaktur.map((spk) => (
                      <tr key={spk.id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3 px-3 font-mono font-bold text-blue-600">{spk.no_spk}</td>
                        <td className="py-3 px-3 font-bold text-slate-900">{spk.no_polisi}</td>
                        <td className="py-3 px-3 text-slate-700">{spk.nama_customer || '-'}</td>
            ...
            <div className="space-y-3">
              {invoiceUnpaid.length > 0 ? (
                invoiceUnpaid.map((inv) => (
                  <div key={inv.id} className="p-3 rounded-xl border border-amber-200 bg-amber-50/40 space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-mono font-bold text-blue-700">{inv.no_invoice}</span>
                    ...
                    <div className="text-xs font-black text-amber-900 pt-1 border-t border-amber-100 flex items-center justify-between">
                      <span>Total:</span>
  ...
  const renderPicTerkaitDashboard = () => {
    ...
    return (
      <div className="space-y-6">
        {/* Banner */}
        <div className="rounded-2xl bg-gradient-to-r from-teal-700 via-cyan-700 to-teal-800 text-white p-5 sm:p-6 shadow-md relative overflow-hidden">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/20 text-white text-[11px] font-semibold mb-2 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-2.5">
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-white text-teal-700 font-bold text-xs shadow-sm hover:bg-teal-50 transition-all flex items-center gap-1.5"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-rose-600 uppercase bg-rose-50 px-2 py-0.5 rounded-md">Gate Pos</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{tamuMenunggu.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Tamu Menunggu Konfirmasi</div>
          </div>

          <div 
            onClick={() => setActiveTab('pic-terkait')}
            className="bg-white rounded-2xl p-4 border border-teal-200 shadow-xs hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
              <div className="w-8 h-8 rounded-xl bg-teal-500 text-white flex items-center justify-center">
                <UserCheck className="w-4 h-4" />
              </div>
              <span className="text-[10px] font-bold text-teal-600 uppercase bg-teal-50 px-2 py-0.5 rounded-md">Di Lokasi</span>
            </div>
            <div className="text-2xl font-black text-slate-900">{tamuAktif.length}</div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Tamu Sedang di Area Bengkel</div>
          </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] font-bold text-emerald-600 uppercase bg-emerald-50 px-2 py-0.5 rounded-md">Selesai</span>
            </div>
            <div className="text-2xl font-black text-slate-900">
              {antrianList.filter((a) => a.tujuan_kedatangan === 'Kunjungan' && a.waktu_keluar).length}
            </div>
            <div className="text-xs font-semibold text-slate-600 mt-0.5">Kunjungan Selesai Hari Ini</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-500">Petugas Security di gerbang membutuhkan konfirmasi penerimaan Anda</p>
            </div>
            <button
              onClick={() => setActiveTab('pic-terkait')}
          ...
          <div className="space-y-3">
            {tamuMenunggu.length > 0 ? (
              tamuMenunggu.map((tamu) => (
                <div key={tamu.id} className="p-4 rounded-xl border border-rose-200 bg-rose-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-bold text-blue-700">{tamu.no_tiket}</span>
                      <span className="font-bold text-slate-900 text-sm">{tamu.no_polisi}</span>
                      <span className="text-xs font-semibold text-slate-700">({tamu.nama_customer || 'Tamu'})</span>
                    </div>
                    <p className="text-xs text-slate-600">
                      Keperluan: <span className="font-semibold text-slate-800">{tamu.keperluan || 'Kunjungan dinas / meeting'}</span>
                    </p>
                    <div className="text-[11px] text-slate-400">
                  ...
                  <button
                    onClick={() => setActiveTab('pic-terkait')}
                    className="px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs shadow-xs transition-colors shrink-0"
                  >
                    Konfirmasi Masuk
                  </button>
                </div>
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3 rounded-2xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-700">Tampilan Dashboard:</span>
          <span className="px-2 py-0.5 rounded-lg bg-blue-50 text-blue-700 font-black text-xs border border-blue-200">
