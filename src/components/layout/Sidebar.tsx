...
export const Sidebar: React.FC = () => {
  ...
  const allNavigationItems: NavItem[] = [
    ...
    { id: 'sa-list', label: 'Daftar SPK Aktif (SA)', icon: ClipboardList, roles: ['Super Admin'] },
    { id: 'sa-baru', label: 'Buat SPK Baru (SA)', icon: PlusCircle, roles: ['Super Admin'] },
    { id: 'sa-kotak-merah', label: 'Kotak Merah PR (SA)', icon: ShoppingBag, roles: ['Super Admin'] },
    ...
    // SA (Service Advisor - 3 Alur Utama + Penjualan Part)
    { id: 'sa-list', label: '📋 Daftar SPK Aktif', icon: ClipboardList, roles: ['SA'] },
    { id: 'sa-baru', label: '➕ Buat SPK Baru', icon: PlusCircle, roles: ['SA'] },
    { id: 'sa-kotak-merah', label: '📦 Kotak Merah (PR)', icon: ShoppingBag, roles: ['SA'] },
      ...
      <div>
        ...
        <nav className="space-y-1">
          {roleMenus.map((item) => {
            ...
            // Prefix-match & alias match agar sub-tab deep-link tetap menyorot menu yang tepat
            const isActive =
              activeTab === item.id ||
              activeTab.startsWith(`${item.id}-`) ||
              (item.id === 'sa-list' && (activeTab === 'sa' || activeTab === 'sa-list')) ||
              (item.id === 'sa-baru' && (activeTab === 'sa-penerimaan' || activeTab === 'sa-baru')) ||
              (item.id === 'sa-kotak-merah' && (activeTab === 'sa-kotak-merah' || (currentRole === 'SA' && activeTab === 'purchasing')));
